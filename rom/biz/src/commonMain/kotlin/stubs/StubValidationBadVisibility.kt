...
fun ICorChainDsl<Context>.stubValidationBadVisibility(title: String) = worker {
    this.title = title
    this.description = """
        Кейс ошибки валидации для видимости модели
    """.trimIndent()

    on { stubCase == Stubs.BAD_VISIBILITY && state == State.RUNNING }
    handle {
        fail(
            ROMError(
                group = "validation",
