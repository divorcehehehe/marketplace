...
class MapperTrainTest {
    @Test
    fun fromTransport() {
        val request = ModelTrainRequest(
            debug = ModelDebug(
        ...
        assertEquals("TRAIN",    context.command.name)
    ...
    fun toTransport() {
        ...
        val request = context.toTransportModel() as ModelTrainResponse

        assertEquals("train",            request.responseType)
